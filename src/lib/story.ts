...
// ============================================================
// Prefetch cache — Adventure Pass users only.
// Speculatively generates the next scene for the most likely
// choice while the user is reading. Keyed by story + scene + choice.
// ============================================================

type PrefetchedScene = { parsed: Scene; text: string };
const prefetchCache = new Map<string, PrefetchedScene>();
const prefetchInFlight = new Map<string, Promise<void>>();

const prefetchKey = (storyId: string, sceneCount: number, choiceId: string) =>
  `${storyId}:${sceneCount}:${choiceId}`;

export const clearPrefetchCache = (storyId?: string) => {
  if (!storyId) {
    prefetchCache.clear();
  ...
  for (const key of Array.from(prefetchCache.keys())) {
    if (key.startsWith(`${storyId}:`)) prefetchCache.delete(key);
  }
  for (const key of Array.from(prefetchInFlight.keys())) {
    if (key.startsWith(`${storyId}:`)) prefetchInFlight.delete(key);
  }
};

export const consumePrefetchedScene = (
  storyId: string,
  sceneCount: number,
  choiceId: string,
): PrefetchedScene | null => {
  const key = prefetchKey(storyId, sceneCount, choiceId);
  const hit = prefetchCache.get(key);
  if (hit) {
    prefetchCache.delete(key);
    console.log(`⚡ Prefetch HIT for choice ${choiceId} at scene ${sceneCount}`);
    return hit;
  }
  return null;
};

export const prefetchNextScene = async (
  profile: Profile,
  currentScene: Scene,
  choiceId: string,
  ...
  storyId: string,
  availableAbilities: string[] = [],
  storyMemory?: { flags: string[]; pastChoices: string[] },
): Promise<void> => {
  if (!storyId) return;
  const key = prefetchKey(storyId, nextSceneCount - 1, choiceId);
  if (prefetchCache.has(key) || prefetchInFlight.has(key)) return;

  const run = (async () => {
    try {
      const sceneWithMemory = {
        ...currentScene,
        selectedChoiceId: choiceId,
        memory: storyMemory ?? { flags: [], pastChoices: [] },
      };
      ...
      if (result.parsed) {
        prefetchCache.set(key, { parsed: result.parsed, text: result.text });
        console.log(`✨ Prefetched scene for choice ${choiceId} at scene ${nextSceneCount - 1}`);
      }
    } catch (e) {
      console.log("Prefetch failed (non-fatal):", e);
    } finally {
