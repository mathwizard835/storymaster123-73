...
|                     |                                                                                                                           |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **Component**       | **What to double-check**                                                                                                  |
| **UX & Delivery**   | Treat frontend status as "Queued" unless the queue worker confirms delivery.                                              |
| **Inbound Context** | Ensure admins know where customer follow-up emails will actually land.                                                    |
| **Database RLS**    | Ensure `support_replies` has `SELECT` and `INSERT` policies tied to `has_role(auth.uid(), 'admin')`.                      |
| **Formatting**      | Run the plain-text/HTML body through basic sanitization to prevent accidental rendering issues in recipient mail clients. |
